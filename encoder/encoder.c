    ...
    x264_picture_t* pic_out)
{
    ...
    *pp_nal = NULL;
    ...
    if (h->fenc->b_keyframe)
    {
        // First, generate SEI Picture Timing
        ...
        // Now, generate SEI Recovery Point after SEI Picture Timing
        ...
        // Continue with other SEI messages
        if (h->param.mastering_display.b_mastering_display)
        {
            nal_start(h, NAL_SEI, NAL_PRIORITY_DISPOSABLE);
    ...
    // Handle non-keyframe cases for SEI pic_timing, if needed
    if (!h->fenc->b_keyframe && (h->sps->vui.b_pic_struct_present || h->sps->vui.b_nal_hrd_parameters_present))
    {
        nal_start(h, NAL_SEI, NAL_PRIORITY_DISPOSABLE);
        x264_sei_pic_timing_write(h, &h->out.bs);
        if (nal_end(h))
