...
void x264_sps_write( bs_t *s, x264_sps_t *sps )
{
    ...
    if( sps->b_vui )
    {
        ...
        if( sps->vui.b_nal_hrd_parameters_present )
        {
            ...
            bs_write(s, 5, sps->vui.hrd.i_time_offset_length + 28);
        }
...
void x264_sei_pic_timing_write(x264_t* h, bs_t* s) {
    ...
    bs_t q;
    ALIGNED_4(uint8_t tmp_buf[100]);
    M32(tmp_buf) = 0; // suppress compiler warnings
    bs_init(&q, tmp_buf, 100);
    ...
    if (sps->vui.b_pic_struct_present) {
        bs_write(&q, 4, h->fenc->i_pic_struct - 1);

        for (int i = 0; i < num_clock_ts[h->fenc->i_pic_struct]; i++) {
            if (h->param.b_timecode) {
                bs_write1(&q, 1); 
    
                if (h->param.timecode_str) {
                    int hours, minutes, seconds, frames;
                    sscanf(h->param.timecode_str, "%d:%d:%d:%d", &hours, &minutes, &seconds, &frames);

                    // Write the timecode data
                    bs_write(&q, 2, 0);         // ct_type = 0
                    bs_write1(&q, 1);           // nuit_field_based_flag = 1
                    bs_write(&q, 5, 1);         // counting_type = 1
                    bs_write1(&q, 0);           // full_timestamp_flag = 0
                    bs_write1(&q, 0);           // discontinuity_flag = 0
                    bs_write1(&q, 0);           // cnt_dropped_flag = 0
                    bs_write(&q, 8, frames);    // Frame

                    bs_write1(&q, 1);           // seconds_flag = 1
                    bs_write(&q, 6, seconds);   // Seconds
                    bs_write1(&q, 1);           // minutes_flag = 1
                    bs_write(&q, 6, minutes);   // Minutes
                    bs_write1(&q, 1);           // hours_flag = 1
                    bs_write(&q, 5, hours);     // Hours

                    bs_write(&q, 28, 0);        // time_offset
                }
            }
            else {
                bs_write1(&q, 0);
